...
print(Fore.LIGHTYELLOW_EX + '- |' + Fore.WHITE + ' Welcome to' + Fore.YELLOW + ' BloxFlip' + Fore.BLUE + ' Rain' + Fore.BLUE + ' Notifier ' + Fore.LIGHTYELLOW_EX + '| -');
  ...
  try:
    ...
    if check['active'] == True:
      if check['prize'] >= minimum:
        ...
        if webhook_enable == "True":
          userid = requests.get(
            f"https://api.roblox.com/users/get-by-username?username={host}"
          ).json()['Id']
          thumburl = (
            f"https://www.roblox.com/headshot-thumbnail/image?userId={userid}&height=50&width=50&format=png"
          )
          embed = DiscordEmbed(title=f"{host} is hosting a chat rain / Click to join!",
